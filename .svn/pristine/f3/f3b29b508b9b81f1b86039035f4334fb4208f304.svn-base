...
public class UnpackServiceImpl implements UnpackService {
	...
	public Integer insertUnpack(HisiUnpackInfo hisiUnpackInfo) {
		...
		// }
		for (String e : goodsPics) {
			String url = e.substring(e.indexOf(ftpPic) + ftpPic.length());
			hisiUnpackGoodsPicMapper.updateUnpackId(hisiUnpack.getId(), url);
		}
	...
	String getImageStr(String imgFile) {
		...
		String imageUrl=Base64.getEncoder().encodeToString(data);
		return imageUrl;
	}

	@Override
	public List<String> getUnpackPhoto(String orderId,String startTime,String endTime) {
		return hisiUnpackMapper.getUnpackPhoto(orderId, startTime, endTime);
	}
	...
	@Override
	public List<HisiUnpackNecessaryPic> findNecessaryPic(String orderId,
			String channelId) {
		return hisiUnpackNecessaryPicMapper.findNecessaryPic(orderId,channelId);
	}
}